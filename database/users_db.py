...
from database.parmissions_db import user_roles
from validators.date import validate_not_future_date
...
class UserSchema(Schema):
    ...
    email = fields.Email(required=True, validate=[
                         validate.Length(min=1, max=50), validate.Email()])
    discord_token = fields.Str(validate=validate.Length(max=100))
    password = fields.Str(
        required=True, validate=validate.Length(min=8, max=100))
    token = fields.Str(
        required=False, validate=validate.Length(min=1, max=300))
    creation_date = fields.DateTime(
        dump_only=True, validate=validate_not_future_date)
